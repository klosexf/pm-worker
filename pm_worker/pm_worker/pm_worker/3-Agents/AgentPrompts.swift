...
nonisolated enum AgentPrompts {
    ...
    // MARK: - 非阶段轮：寒暄轻量轮（2026-09-22）

    /// 寒暄/致谢轮的冻结段：用户在打招呼或道谢，不是在提需求。
    /// 刻意**不拼**任何阶段协议段（自评审 / 计划块 / 决策块 / 歧义 / 变更提案 /
    /// 跳转请求全不上）——实测一句「你好」在 ③ 阶段背着 18k 字符骨架与「每轮必须
    /// 自评审」的硬命令，被逼着给寒暄逐项填表，付了 22,463 token 输入 × 2 次
    /// 串行调用 = 127 秒。禁令必须显式写：只靠「不要求」拦不住模型在阶段轮
    /// 养成的补块习惯（那 127 秒的原始思考链就是在填表）。
    static let smallTalkReply: String = """
        你是面向产品经理的需求工作流助手。此刻用户只是在打招呼或表达谢意，没有提出任何工作诉求。

        回应要求：
        - 一两句中文，自然简短，像同事之间的应答，不要客套堆砌。
        - 用户若是问候，可顺带一句你能做什么（澄清需求 / 画功能结构 / 出交互原型 / 写需求文档），一句话点到即止。
        - 禁止输出任何以 ```artifact 开头的代码块：本轮没有产物，也没有需要自评的东西。
        - 禁止调用任何工具（联网检索、技能加载、记忆读写等一律不用），直接作答。
        - 不要列清单、不要分小节、不要加标题。
        """
    ...
    static func prd(
        ...
        3. **数据指标与口径**：数字 100% 来自用户对话中的明确表述；用户未给数值的指标写\
        「实施期建立基线」并标注基线待建立；缺口径又无法起草的指标写入附录「待定问题清单」。\
        每条量化指标除数字外还必须给出**口径**（分子 / 分母 / 统计时间窗三项为核心，\
        另可给数据来源与排除条件），口径决定三个月后这个数字能不能和上线数据对上：\
        用户讲过口径的照写、status 记 `confirmed`；用户只给数字没讲口径的，\
        **可以**按常识起草候选口径、status 记 `assumed` 并把起草了什么假设写进 `assumptionNote`\
        （严禁把起草的口径当成既成事实），同时按「PRD 默认项卡」出口径题让用户一键确认或修改；\
        口径缺到无法起草的 status 记 `pending`。\
        **口径的文字由你写进下面这个记录块，不要再抄一遍到正文**：PRD 4.2 每条指标的口径\
        位置只写一个引用槽 `[[SPEC:指标名]]`（占在该指标的口径单元格 / 行内，前后不要再补\
        任何口径描述），落盘时 App 会用记录块的内容渲染替换——这样正文与记录只有一个作者，\
        不可能出现「正文写一种分母、记录写另一种分母」。记录块本身仍不许出现基线值 / 目标值\
        / 测量时点这些数字（它们只活在 4.2 表其余列，多一份数字就是第二份事实源）。\
        记录块位置：PRD 产物块闭合之后、自评块之前；本版无量化指标则不出该块。

        ```artifact:metric-specs
        [{"name":"次日留存率","dataSource":"App 埋点 daU 表","numerator":"注册次日仍打开 App 的去重用户数",
          "denominator":"当日注册新增用户数","window":"自然日 00:00-24:00，用户按 device_id 去重",
          "exclusions":"剔除内部测试账号与当日卸载重装","status":"assumed",
          "assumptionNote":"去重键与统计窗口按常见做法起草，待确认"}]
        ```

        一致性要求：`[[SPEC:…]]` 里的指标名、记录块的 `name`、4.2 表首列三处必须逐字一致，\
        且条数相等——每个槽都要被渲染掉，正文残留 `[[SPEC:` 即缺陷。
    ...
    static func prdDefaultsCardSection() -> String {
        ...
        **不再用列表逐条罗列默认项**（卡片承载，避免重复），\
        ...
        3. **指标口径优先占位**：凡 `metric-specs` 块里 status 为 `assumed` / `pending` 的指标，\
        必须出一题（多个指标可合并成一题问「以下 N 条指标的分子/分母/时间窗按此起草，是否准确」，\
        选项里逐条列出被起草的口径）——口径是三个月后对账的唯一依据，默认掉就等于这笔账永远对不上；\
        用户确认后重出 PRD 时，把对应指标 status 改记 `confirmed` 并清空 `assumptionNote`；
        4. 用户答案将以【问题卡作答】消息回传：收到后按答案修订 PRD 并重新输出完整 \
        artifact:prd 产物块（未涉及的章节原样保留），radar / decision 自评块照常输出；\
        **不再重复输出默认项卡**（除非修订中出现新的实质缺省项）；
        5. 跳过 / 保持默认的项维持现状，并留在 PRD 附录「待定问题清单」；
        6. 出默认项卡的回复同样不再追加 A) 选项行。
        """
    }
    ...
    static func stageChecklist(_ stage: String) -> String {
        ...
        case "prd":
            return "④ PRD：功能需求与已确认结构及原型一一对应；13 章骨架完整（lean 档一章不少）；需求概述首句为目标收敛句；详细设计每页 7 子项齐（lean 档每页概览/行为规则/验收 eval 3 项）、行为规则表为唯一事实来源、状态完整性检验与穷举三步法已执行；引用出处齐全；无上游输入外新假设；功能架构图（6.1）与核心流程图（8.1）已内嵌且与已确认结构产物一致；每页配 ASCII 布局图；指标口径可追溯（4.2 每条量化指标都有口径或已标待定并出现在附录「待定问题清单」；起草的口径已标注为假设、未冒充用户既定事实；已登记口径记录与 4.2 表是同一批指标、条数一致，且该记录内不含任何数值；正文不得残留未渲染的 `[[SPEC:` 引用槽）。"
        default:

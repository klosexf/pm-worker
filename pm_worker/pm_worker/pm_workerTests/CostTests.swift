...
final class CostTests: XCTestCase {
    ...
    func testGLMTierByContext() {
        ...
        let hitCost: Double = 30_000.0 / 1_000_000 * 0.8
        let missCost: Double = 2_769.0 / 1_000_000 * 4
        let outputCost: Double = 1_000.0 / 1_000_000 * 16
        XCTAssertEqual(large, hitCost + missCost + outputCost, accuracy: 1e-9)
    }

    func testFlatModelCost() {
        // 平价模型：无峰谷（peak 参数无效）、无缓存档（输入全按 input 单价 21，忽略命中数）
        ...
        XCTAssertEqual(cost, 21 + 0.5 * 105, accuracy: 1e-9)
    }
    ...
    func testFreeModelNotUnknown() {
        let tracker = makeTracker()
        record(
            tracker, ts: "2026-09-01T10:00:00+08:00", model: "qwen2.5",
            prompt: 9_999, completion: 9_999
        )
        let session = tracker.session
    ...
    func testLegacyJSONLWithoutCacheHitField() throws {
        ...
        XCTAssertEqual(
            month.costCNY, 1000.0 / 1_000_000 * 2 + 500.0 / 1_000_000 * 8, accuracy: 1e-9
        )
    }
    ...
    func testParseStreamUsageCacheHit() throws {
        ...
        """
        let noneUsage = try XCTUnwrap(LLMClient.parseUsage(fromPayload: Data(none.utf8)))
        XCTAssertEqual(noneUsage.effectiveCacheHitTokens, 0)
    }

...
> **本地优先的 macOS AI 产品经理副驾——澄清 → 结构 → 原型 → PRD 四阶段流水线，人在环确认闸口。自带模型 Key（BYOK），无账号、无遥测。**
...
- **人在环确认闸口，不是自动驾驶**。澄清要点、结构、原型各是一道闸口，不确认不推进；PRD 只能描述你确认过的结构与原型——无凭空功能。
...
本项目文档驱动开发：[PRD.md](PRD.md)（产品需求，26 条验收标准）、[design.md](design.md)（架构与 27 条 Eval、决策记录）、[skills-inventory.md](skills-inventory.md)（技能清单）都在仓库内。English README: [README.md](README.md)。

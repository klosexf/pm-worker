...
| M5 | MCP server（6 工具）+ 状态页、双语 README、演示物料、全量 E2E 回归与 M5 验收线 | **进行中**（本 README 即 M5 交付物） |
| **M6 · 第二圈** | **闭环**：事实入口 → 结果对账 → 项目级需求池 → 开工档案袋 | **规划中 · 未排期**（架构与落点已预留六处咬合点，见 design.md §14） |

**北极星（产品级、唯一）是「跑通完整闭环的项目数」——当前为 0，因为第二圈还没做。** 上表 M5 那行是**里程碑验收线**，不是北极星；两者在 PRD §4.2 已拆为两层（此前同一张表混填，结果是第二圈在指标面上永不可见）。

产品另有与闭环正交、按需插拔的旁支：本地数据计算（CSV + Python）、原型第三方导出通道、技能「学」模式、飞书 / Notion 导出。

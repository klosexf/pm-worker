...
面向 AI 编码代理的项目约定。动手前先读完本文件与根目录 `bugs.md`（高成本 bug 经验库），可避免本仓库已付出过学费的所有坑。
...
在所有面向用户的信息展示场景中（包括但不限于AI回答内容、各类展示卡片、知识库条目及其他用户可见界面元素），必须严格遵守以下内容过滤规则：

1. **禁止展示产品内部实现细节**：
   - 不得包含任何关于该产品的产品逻辑实现方式的说明
   - 不得透露系统决策机制或工作原理

2. **禁止展示技术相关内容**：
   - 不得包含任何形式的代码片段（包括但不限于代码示例、伪代码、配置文件内容）
   - 不得使用该产品的技术术语、开发框架名称、编程语言名称等专业技术词汇
   - 不得展示该产品的系统架构、模块划分等技术设计相关内容
...
- **每次会话开始必读根目录 `bugs.md`**（高成本 bug 经验库：难解、耗时数小时、反复出现的 bug 排查记录）。排查任何 bug 前先对照其中条目，症状命中直接采用已验证解法，禁止重复排查。
- **bugs.md 写入闭环**：解决了一个「难解 / 耗时数小时 / 反复出现」的 bug 后，必须按 `bugs.md` 内的模板追加条目（症状 / 根因 / 解法等固定字段）；同一 bug 复发时更新原条目的状态、复发次数与最新解法，不新开条目。
- **文档单一事实源**：活文档只有 `PRD.md`（需求口径）与 `design.md`（技术口径），两者变更时在各自文档头追加版本化变更记录（这份记录就是本仓库的决策日志，检索用 `git log -S"<新小节标题>"`，不依赖提交信息）。根目录其余 `PM Agent · 简化版需求文档*` / `PM 全流程 AI Agent*` / `PM Copilot 产品全景文档*` 等一律视为**历史存档或可再生快照**（三条在 `.gitignore` 里**按文件名逐条排除**，非临时起意；全景文档那条见于 `0019adc`）：**改 PRD/design 时不去同步它们，也不得从它们取现行口径**；需要对外评审材料时从 PRD + design 现场重新生成、用完即弃。同一原则的架构版是「文件是唯一事实源、SQLite 只是可重建索引」——衍生品不留副本。同理 `evals/results/`（每次 live 跑重新生成）。
- **代码改动必须回写需求文档**：改完产品代码方案（功能行为、阶段流水线、数据结构、对外 MCP 工具、验收口径等任何影响「产品是什么」的实现）后，必须回头核对 `PRD.md`（需求口径）与 `design.md`（技术口径）是否仍与代码一致；发现口径已漂移就**在同一次改动里同步更新文档**（并按上条在文档头追加版本化变更记录），不留「代码已变、文档还写旧方案」的状态。只做纯内部重构（不改外部行为与口径）时可不动文档，但要在提交信息里说明为何无需同步。
